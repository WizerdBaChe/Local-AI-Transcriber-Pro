...
class HomePage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(self, text="AI Studio Launcher", font=ctk.CTkFont(family="Segoe UI", size=36, weight="bold"),
                     text_color="#1C1C1E").grid(row=0, column=0, pady=(100, 10))
        ctk.CTkLabel(self, text="全方位語音分析與知識視覺化工具", font=ctk.CTkFont(size=16), text_color="#8E8E93").grid(
            row=1, column=0, pady=(0, 60))

        self.cards_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.cards_frame.grid(row=2, column=0)

        self.create_card("▶ YouTube 轉錄", "自動下載網址並產出逐字稿", "#007AFF",
                         lambda: controller.show_frame("YouTubePage"))
        self.create_card("📁 本地分析", "處理電腦中的影音檔案", "#34C759", lambda: controller.show_frame("LocalPage"))
        self.create_card("🧠 心智圖生成", "將 JSON 轉為視覺化圖譜", "#AF52DE",
                         lambda: controller.show_frame("MindMapPage"))
...
class YouTubePage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(header, text="YouTube 影片轉錄", font=ctk.CTkFont(family="Segoe UI", size=28, weight="bold"),
                     text_color="#1C1C1E").pack(side="left")
        ctk.CTkButton(header, text="← 返回首頁", width=100, height=32, fg_color="transparent", text_color="#007AFF",
        ...
        self.entry_url = ctk.CTkEntry(input_card, placeholder_text="在此貼上影片網址...", height=45, fg_color="#F2F2F7",
                                      border_width=0, text_color="#1C1C1E")
        ...
        self.btn_run = ctk.CTkButton(btn_frame, text="🚀 開始轉錄", command=self.start_task, height=40, width=140,
                                     fg_color="#007AFF")
        ...
        self.status_label = ctk.CTkLabel(log_card, text="系統待命", text_color="#34C759",
        ...
        self.textbox = ctk.CTkTextbox(log_card, font=ctk.CTkFont(family="Consolas", size=13), fg_color="#F9F9F9",
                                      text_color="#1C1C1E", border_spacing=10)
...
class LocalPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        header = ctk.CTkFrame(self, fg_color="transparent")
        header.grid(row=0, column=0, sticky="ew", padx=40, pady=(40, 20))
        ctk.CTkLabel(header, text="本地多媒體轉錄", font=ctk.CTkFont(family="Segoe UI", size=28, weight="bold"),
                     text_color="#1C1C1E").pack(side="left")
        ctk.CTkButton(header, text="← 返回首頁", width=100, height=32, fg_color="transparent", text_color="#007AFF",
                      command=lambda: controller.show_frame("HomePage")).pack(side="right")
        ...
        self.btn_browse = ctk.CTkButton(file_box, text="📁 選擇檔案", command=self.browse_file, width=120, height=40,
                                        fg_color="#34C759")
        ...
        self.btn_run = ctk.CTkButton(btn_row, text="🚀 開始分析", command=self.start_task, height=40, width=140,
                                     state="disabled")
        self.btn_run.pack(side="left")
        self.btn_stop = ctk.CTkButton(btn_row, text="🛑 停止", command=self.stop_task, height=40, width=90,
                                      fg_color="#FFE5E5", text_color="#D70015", state="disabled")
        self.btn_stop.pack(side="left", padx=15)
        ...
        self.progress_bar = ctk.CTkProgressBar(log_card, height=8)
        self.progress_bar.grid(row=1, column=0, sticky="ew", padx=25, pady=(20, 15))
        self.progress_bar.set(0)
    ...
    def browse_file(self):
        path = filedialog.askopenfilename(title="選擇檔案", filetypes=(("媒體檔案", "*.mp3 *.wav *.mp4 *.m4a *.mkv"),
                                                                       ("所有檔案", "*.*")))
        if path:
    ...
    def work_thread(self):
        success = process_local_file(
            ...
            log_cb=self.log, progress_cb=self.progress_bar.set, stop_event=self.stop_event
        )
        self.is_processing = False
        self.btn_run.configure(state="normal")
        self.btn_stop.configure(state="disabled")
...
class MindMapPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        header = ctk.CTkFrame(self, fg_color="transparent")
        header.grid(row=0, column=0, sticky="ew", padx=40, pady=(40, 20))
        ctk.CTkLabel(header, text="心智圖生成", font=ctk.CTkFont(family="Segoe UI", size=28, weight="bold"),
                     text_color="#1C1C1E").pack(side="left")
        ctk.CTkButton(header, text="← 返回首頁", width=100, height=32, fg_color="transparent", text_color="#007AFF",
                      command=lambda: controller.show_frame("HomePage")).pack(side="right")
        ...
        self.btn_browse = ctk.CTkButton(file_box, text="📂 選擇 JSON", command=self.browse_json, fg_color="#AF52DE")
        self.btn_browse.pack(side="left", padx=15, pady=15)
        ...
        btn_row = ctk.CTkFrame(input_card, fg_color="transparent")
        btn_row.pack(fill="x", padx=25, pady=(0, 25))
        self.btn_gen = ctk.CTkButton(btn_row, text="🪄 生成 HTML 心智圖", command=self.generate_map, height=40,
...
class ClientApp(ctk.CTk):
    def __init__(self):
        ...
        self.sidebar = ctk.CTkFrame(self, width=240, corner_radius=0, fg_color="#FFFFFF", border_width=1,
                                    border_color="#E5E5EA")
        self.sidebar.grid(row=0, column=0, sticky="nsew")
        self.sidebar.grid_rowconfigure(6, weight=1)

        ctk.CTkLabel(self.sidebar, text="AI STUDIO", font=ctk.CTkFont(family="Segoe UI", size=24, weight="bold"),
                     text_color="#007AFF").grid(row=0, column=0, padx=20, pady=(40, 40))
        ...
        ctk.CTkLabel(self.sidebar, text="轉錄引擎設定", text_color="#8E8E93", font=ctk.CTkFont(size=12)).grid(row=7,
                                                                                                              column=0,
                                                                                                              padx=20,
                                                                                                              pady=(10,
                                                                                                                    5),
                                                                                                              sticky="w")
        self.model_option = ctk.CTkOptionMenu(self.sidebar, values=["base", "small", "medium", "large-v3"],
                                              fg_color="#F2F2F7", text_color="#1C1C1E")
        self.model_option.set("large-v3")
        self.model_option.grid(row=8, column=0, padx=20, pady=(0, 30), sticky="ew")
